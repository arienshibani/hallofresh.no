<script>
    // @ts-nocheck

    export let data;
    import {
        Card,
        MenuButton,
        Dropdown,
        DropdownItem,
        Avatar,
        Button,
    } from "flowbite-svelte";
    import { PlusCircle, MinusCircle } from "svelte-heros-v2";

    function handlePlus() {
        count += 1;
        if (count >= 10) {
            // Do not allow the counter to go above 10
            count = 10;
        }
    }

    function handleMinus() {
        count -= 1;
        // Do not allow the counter to go below 1
        if (count <= 1) {
            count = 1;
        }
    }

    let count = 1;

    // Retrieve the recipes from the data.
    $: ({ recipe } = data);

    console.log(recipe);
</script>

<h1 class="text-5xl text-center pt-16 pb-5">{recipe.dish}</h1>
<h1 class="text-2xl text-center pb-5">{recipe.subtitle}</h1>

<div class="flex justify-center flex-wrap pt-10">
    <div class="">
        <h1 class="text-2xl font-bold">Ingredienser</h1>
        <div class="flex justify-start pt-5">
            <button on:click={handleMinus} class="pr-5">
                <MinusCircle class="inline" />
            </button>
            <h2 class="text-2xl text-gray-900">
                {count}
                {count === 1 ? "Person  " : "Personer"}
            </h2>
            <button on:click={handlePlus} class="pl-5">
                <PlusCircle class="inline" />
            </button>
        </div>
        {#each recipe.dishIngredients as ingredient}
            <p class="pt-6 pr-40 text-xl justify-center">
                {ingredient.amount * count}{ingredient.measurement}
                {ingredient.name}
            </p>
        {/each}
    </div>

    <div class="flex-grow-1">
        <h1 class="text-2xl font-bold pb-5">Slik gjør du</h1>
        {#each recipe.steps as step, index}
            <p class="text-xl p-4 max-w-lg">
                <span class="font-bold">{index + 1 + "."}</span>
                {step}
            </p>
        {/each}
    </div>
</div>

<style>
    .gridsetup {
        display: block;
        font-size: 80%;
    }
</style>
